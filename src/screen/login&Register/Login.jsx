...
function LoginPage() {
    ...
    async function handleSubmit() {
        ...
        const userData = { email:email, 
            password:password };
            axios.post('http://192.168.0.115:5001/login-user', {
                email: email,
                password: password,
              })
              .then(res => {
                ...
                if (res.data.status === 'ok') {
                  ...
                  AsyncStorage.setItem('token', res.data.data); // Save the token to AsyncStorage
                  navigation.navigate('EApp'); // Navigate to EApp
                } else {
                  Alert.alert('Login Failed', res.data.data || 'Login failed. Please try again.');
                }
              })
              .catch(error => {
